### This file is the original tutorial but modified to match Tutoria_modified
...
class StockTradingEnv(gym.Env):
    ...
    def _next_observation(self):
        ...
        frame = np.array([
            self.df.iloc[self.current_step: self.current_step +
                        LOOKBACK_WINDOW_SIZE+1,].loc[:,'Open'].values/ MAX_SHARE_PRICE,
            self.df.iloc[self.current_step: self.current_step +
                        LOOKBACK_WINDOW_SIZE+1,].loc[:,'High'].values/ MAX_SHARE_PRICE,
            self.df.iloc[self.current_step: self.current_step +
                        LOOKBACK_WINDOW_SIZE+1,].loc[:,'Low'].values / MAX_SHARE_PRICE,
            self.df.iloc[self.current_step: self.current_step +
                        LOOKBACK_WINDOW_SIZE+1,].loc[:,'Close'].values/ MAX_SHARE_PRICE,
            self.df.iloc[self.current_step: self.current_step +
                        LOOKBACK_WINDOW_SIZE+1,].loc[:,'Volume'].values/ MAX_NUM_SHARES,
        ])
    ...
    def _take_action(self, action):
        ...
        if action_type < 1:
            ...
            number_purchase = int(total_possible*amount)
        ...
        # Sell
            ...
            number_sell = int(self.held * amount)
            total_sell = current_price * number_sell
            self.held -= number_sell
            self.balance += total_sell

            ##
            # self.total_shares_sold += number_sell
            # self.total_sales_value += number_sell * current_price
            ##
    ...
    def _render_to_file(self, filename='render.txt'):
        ...
        file.write(
            f'Shares held: {self.held} (Total sold: {self.total_shares_sold})\n')
        file.write(
...
class StockTradingGraph:
    ...
    def _render_price(self, current_step, net_worth, dates, step_range):
        ...
        candlestick(self.price_ax, candlesticks, width=0.05,
    ...
    def _render_volume(self, current_step, net_worth, dates, step_range):
        ...
        self.volume_ax.bar(dates[pos], volume[pos], color=UP_COLOR,
                           alpha=0.4, width=0.05, align='center')
